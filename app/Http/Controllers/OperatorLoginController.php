...
use App\operatorLogin_wh;
use App\operatorLogin_bon;
use App\operatorLogin_sec;
use App\activity;
			...
			if (isset($inteos[0]->Name)) {
				...
				if (isset($alredy_logged_today[0]->id)) {
					...
					if (isset($alredy_logged_today_not_closed_activity[0]->id)){
						...
						$activity = (object) $activity;
						// dd($activity);
						$msg3 = 'User already have logged today and previous acivity was not finished, please use function change activity or logout.';
						return view('OperatorLogin.login' , compact('activity', 'msg3'));
					}
				...
				$department_search = DB::connection('sqlsrv')->select(DB::raw("SELECT rnumber FROM cutting_department WHERE rnumber = '".$rnumber."' "));
				// dd('$shift_name');
				if (isset($department_search[0]->rnumber)) {
					$department = 'Cutting';
				} else {
					$department = 'Others';
				}
	...
	// Login KIK
		...
		public function operator_login_new_post_kik(Request $request) {
			...
			if (isset($inteos[0]->Name)) {
				...
				// Determine the login date
				if ($currentHourMinute >= '00:00' && $currentHourMinute < '04:00') {
				    // Between 00:00 and 06:10, use the previous day as login date
				    $login_date = $currentTime->modify('-1 day')->format('Y-m-d');
				    $shift_name = '3';
				} else {
				...
				// Determine shift_start based on the specified logic
				if ($currentHourMinute >= '06:00' && $currentHourMinute <= '07:00') {
				    
				    $shift_start = $login_date . ' 07:00';
				    $shift_name = '1';
				} elseif ($currentHourMinute >= '07:00' && $currentHourMinute <= '15:00') {
...
// WH Subotica
	// Table
		public function table_operator_new_wh() {
			$data = DB::connection('sqlsrv')->select(DB::raw("SELECT op.*
					,a.activity_desc
					,a.activity_desc1

				FROM [preproduction].[dbo].[operator_login_wh] as op
				JOIN [preproduction].[dbo].[activity_wh] as a ON a.id = op.activity_id

				WHERE  op.created_at >= DATEADD(day,-30,GETDATE()) 
				ORDER BY op.created_at desc
			"));

			return view('OperatorLogin.table_wh', compact('data'));
		}

	// Login
		public function login_wh()	{
			//

			$activity = DB::connection('sqlsrv')->select(DB::raw("SELECT id, activity_desc FROM activity_wh ORDER BY id asc"));
			$activity = (object) $activity;
			// dd($activity);
			...
			return view('OperatorLogin.login_wh' , compact('activity'));
		}

		public function operator_login_new_post_wh(Request $request) {
			// $this->validate($request, ['rnumber'=>'required','activity'=>'required']);
			...
			if ($activity_id == "0") {
				...
				// dd($activity);
				$msg3 = 'Aktivnost mora biti izabrana!';
				return view('OperatorLogin.login_wh' , compact('activity', 'msg3'));
			}
			...
			if (isset($inteos[0]->Name)) {
				...
				$alredy_logged_today = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_wh WHERE rnumber = '".$rnumber."' and login_date = '".$login_date."' "));

				if (isset($alredy_logged_today[0]->id)) {
					//user already looged today
					
					// previous activity was not closed
					$alredy_logged_today_not_closed_activity = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_wh WHERE rnumber = '".$rnumber."' and login_date = '".$login_date."' and logout_actual is null "));
				...
				$if_logged_from_before = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_wh WHERE rnumber = '".$rnumber."' and logout_actual is null "));
				if (isset($if_logged_from_before[0]->login_date)) {
					// there is some problem because user was not loged out from before
					dd('there is some problem because user was not loged out from '.substr($if_logged_from_before[0]->login_date,0,10).' ,call IT.');
			...
			// dd($activity);
			$msg4 = 'Operater '.$operator.' uspesno prijavljen';
			return view('OperatorLogin.login_wh' , compact('activity', 'msg4'));
		}

	// Change 
		public function change_wh() {
			//
			...
			return view('OperatorLogin.change_wh' , compact('activity'));
		}

		public function operator_change_new_post_wh(Request $request) {
			...
			if ($activity_id == "0") {
				...
				// dd($activity);
				$msg3 = 'Aktivnost mora biti izabrana!';
				return view('OperatorLogin.change_wh' , compact('activity', 'msg3'));
			}
			...
			if (isset($inteos[0]->Name)) {
				...
				// dd($login_date);
				$alredy_logged_today = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_wh WHERE 
						rnumber = '".$rnumber."' and login_date = '".$login_date."' AND logout_actual is NULL "));
				// dd($alredy_logged_today);
				...
				if (isset($alredy_logged_today[0]->id)) {
					...
					$if_logged_on_same_activity = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_wh
		  					WHERE activity_id = '".$activity_id."' and rnumber = '".$rnumber."' and logout_actual is null  "));
					// dd($if_logged_on_same_activity);
					...
					$table1 = operatorLogin_wh::findOrFail($alredy_logged_today[0]->id);
					// $table1->key_string = $login_date.'-'.$activity_id.'-'.$rnumber;
					// $table1->rnumber = $rnumber;
					// $table1->operator = $operator;
					// $table1->activity_id = $activity_id;
			...
			// dd($activity);
			$msg4 = 'Operater '.$operator.' je uspesno promenio aktivnost';
			return view('OperatorLogin.change_wh' , compact('activity', 'msg4'));

		}

	// Logout
		public function logout_wh() {
			//
			...
			return view('OperatorLogin.logout_wh' /*, compact('activity')*/);
		}

		public function operator_logout_new_post_wh(Request $request) {
			...
			if (isset($alredy_logged_today[0]->id)) {
				...
				// dd($activity);
				$msg4 = 'Operater '.$table1->operator.' se uspesno odjavio';
				return view('OperatorLogin.logout_wh' , compact('activity', 'msg4'));
				...
				return view('OperatorLogin.logout_wh' , compact('activity', 'msg3'));
			}
...
// Bonding 
	// Table
		public function table_operator_new_bon() {
			$data = DB::connection('sqlsrv')->select(DB::raw("SELECT op.*
					,a.activity_desc
					,a.activity_desc1

				FROM [preproduction].[dbo].[operator_login_bon] as op
				JOIN [preproduction].[dbo].[activity_bon] as a ON a.id = op.activity_id

				WHERE  op.created_at >= DATEADD(day,-30,GETDATE()) 
				ORDER BY op.created_at desc
			"));

			return view('OperatorLogin.table_bon', compact('data'));
		}

	// Login
		public function login_bon()	{
			//

			$activity = DB::connection('sqlsrv')->select(DB::raw("SELECT id, activity_desc 
				FROM activity_bon ORDER BY id asc"));
			$activity = (object) $activity;
			// dd($activity);
			...
			return view('OperatorLogin.login_bon' , compact('activity'));
		}

		public function operator_login_new_post_bon(Request $request) {
			// $this->validate($request, ['rnumber'=>'required','activity'=>'required']);
			...
			if ($activity_id == "0") {
				...
				// dd($activity);
				$msg3 = 'Aktivnost mora biti izabrana!';
				return view('OperatorLogin.login_bon' , compact('activity', 'msg3'));
			}
			...
			if (isset($inteos[0]->Name)) {
				...
				// Determine the login date
				if ($currentHourMinute >= '00:00' && $currentHourMinute < '06:30') {
				    // Between 00:00 and 06:10, use the previous day as login date
				    $login_date = $currentTime->modify('-1 day')->format('Y-m-d');
				    $shift_name = '3';
				} else {
				...
				// Determine shift_start based on the specified logic
				if ($currentHourMinute >= '06:30' && $currentHourMinute <= '07:00') {
				    
				    $shift_start = $login_date . ' 07:00';
				    $shift_name = '1';
				} elseif ($currentHourMinute >= '07:00' && $currentHourMinute <= '14:30') {
				    
				    $shift_start = $currentTime->format('Y-m-d H:i');
				    $shift_name = '1';
				} elseif ($currentHourMinute >= '14:30' && $currentHourMinute <= '15:00') {
				    
				    $shift_start = $currentTime->format('Y-m-d') . ' 15:00';
				    $shift_name = '2';
				} elseif ($currentHourMinute >= '15:00' && $currentHourMinute <= '21:45') {
				...
				$alredy_logged_today = DB::connection('sqlsrv')->select(DB::raw("SELECT * 
					FROM operator_login_bon WHERE rnumber = '".$rnumber."' and login_date = '".$login_date."' "));

				if (isset($alredy_logged_today[0]->id)) {
					//user already looged today
					
					// previous activity was not closed
					$alredy_logged_today_not_closed_activity = DB::connection('sqlsrv')->select(DB::raw("SELECT * 
						FROM operator_login_bon WHERE rnumber = '".$rnumber."' and login_date = '".$login_date."' and logout_actual is null "));
					...
					if (isset($alredy_logged_today_not_closed_activity[0]->id)){
						...
						$activity = (object) $activity;
						// dd($activity);
						$msg3 = 'User already have logged today or previous acivity was not finished, please use function change activity or logout.';
						return view('OperatorLogin.login_bon' , compact('activity', 'msg3'));
					}
				...
				$if_logged_from_before = DB::connection('sqlsrv')->select(DB::raw("SELECT * 
					FROM operator_login_bon WHERE rnumber = '".$rnumber."' and logout_actual is null "));
			...
			// dd($activity);
			$msg4 = 'Operater '.$operator.' uspesno prijavljen';
			return view('OperatorLogin.login_bon' , compact('activity', 'msg4'));
		}

	// Change 
		public function change_bon() {
			//
			...
			return view('OperatorLogin.change_bon' , compact('activity'));
		}

		public function operator_change_new_post_bon(Request $request) {
			...
			if ($activity_id == "0") {
				...
				// dd($activity);
				$msg3 = 'Aktivnost mora biti izabrana!';
				return view('OperatorLogin.change_bon' , compact('activity', 'msg3'));
			}
			...
			if (isset($inteos[0]->Name)) {
				...
				if (isset($alredy_logged_today[0]->id)) {
					...
					$if_logged_on_same_activity = DB::connection('sqlsrv')->select(DB::raw("SELECT * 
						FROM operator_login_bon
		  					WHERE activity_id = '".$activity_id."' and rnumber = '".$rnumber."' and logout_actual is null  "));
					// dd($if_logged_on_same_activity);
					...
					$table1 = operatorLogin_bon::findOrFail($alredy_logged_today[0]->id);
					// $table1->key_string = $login_date.'-'.$activity_id.'-'.$rnumber;
					// $table1->rnumber = $rnumber;
					// $table1->operator = $operator;
					// $table1->activity_id = $activity_id;
			...
			// dd($activity);
			$msg4 = 'Operater '.$operator.' je uspesno promenio aktivnost';
			return view('OperatorLogin.change_bon' , compact('activity', 'msg4'));

		}

	// Logout
		public function logout_bon() {
			//
			...
			return view('OperatorLogin.logout_bon' /*, compact('activity')*/);
		}

		public function operator_logout_new_post_bon(Request $request) {
			...
			if (isset($alredy_logged_today[0]->id)) {
				...
				// dd($activity);
				$msg4 = 'Operater '.$table1->operator.' se uspesno odjavio';
				return view('OperatorLogin.logout_bon' , compact('activity', 'msg4'));
				...
				return view('OperatorLogin.logout_bon' , compact('activity', 'msg3'));
			}
...
// SecondQ Audit 
	// Table
		public function table_operator_new_secs() {
			$data = DB::connection('sqlsrv')->select(DB::raw("SELECT op.*
					,a.activity_desc
					,a.activity_desc1

				FROM [preproduction].[dbo].[operator_login_secs] as op
				JOIN [preproduction].[dbo].[activity_secs] as a ON a.id = op.activity_id

				WHERE  op.created_at >= DATEADD(day,-30,GETDATE()) 
				ORDER BY op.created_at desc
			"));

			return view('OperatorLogin.table_secs', compact('data'));
		}

	// Login
		public function login_secs()	{
			//

			$activity = DB::connection('sqlsrv')->select(DB::raw("SELECT id, activity_desc FROM activity_secs ORDER BY id asc"));
			$activity = (object) $activity;
			// dd($activity);
			...
			return view('OperatorLogin.login_secs' , compact('activity'));
		}

		public function operator_login_new_post_secs(Request $request) {
			// $this->validate($request, ['rnumber'=>'required','activity'=>'required']);
			...
			if ($activity_id == "0") {
				...
				// dd($activity);
				$msg3 = 'Aktivnost mora biti izabrana!';
				return view('OperatorLogin.login_secs' , compact('activity', 'msg3'));
			}
			...
			if (isset($inteos[0]->Name)) {
				...
				$alredy_logged_today = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_secs WHERE rnumber = '".$rnumber."' and login_date = '".$login_date."' "));

				if (isset($alredy_logged_today[0]->id)) {
					//user already looged today
					
					// previous activity was not closed
					$alredy_logged_today_not_closed_activity = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_secs WHERE rnumber = '".$rnumber."' and login_date = '".$login_date."' and logout_actual is null "));
				...
				$if_logged_from_before = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_secs WHERE rnumber = '".$rnumber."' and logout_actual is null "));
				if (isset($if_logged_from_before[0]->login_date)) {
					// there is some problem because user was not loged out from before
					dd('there is some problem because user was not loged out from '.substr($if_logged_from_before[0]->login_date,0,10).' ,call IT.');
			...
			// dd($activity);
			$msg4 = 'Operater '.$operator.' uspesno prijavljen';
			return view('OperatorLogin.login_secs' , compact('activity', 'msg4'));
		}

	// Change 
		public function change_secs() {
			//
			...
			return view('OperatorLogin.change_secs' , compact('activity'));
		}

		public function operator_change_new_post_secs(Request $request) {
			...
			if ($activity_id == "0") {
				...
				// dd($activity);
				$msg3 = 'Aktivnost mora biti izabrana!';
				return view('OperatorLogin.change_secs' , compact('activity', 'msg3'));
			}
			...
			if (isset($inteos[0]->Name)) {
				...
				// dd($login_date);
				$alredy_logged_today = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_secs WHERE 
						rnumber = '".$rnumber."' and login_date = '".$login_date."' AND logout_actual is NULL "));
				// dd($alredy_logged_today);
				...
				if (isset($alredy_logged_today[0]->id)) {
					...
					$if_logged_on_same_activity = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM operator_login_secs
		  					WHERE activity_id = '".$activity_id."' and rnumber = '".$rnumber."' and logout_actual is null  "));
					// dd($if_logged_on_same_activity);
					...
					$table1 = operatorLogin_sec::findOrFail($alredy_logged_today[0]->id);
					// $table1->key_string = $login_date.'-'.$activity_id.'-'.$rnumber;
					// $table1->rnumber = $rnumber;
					// $table1->operator = $operator;
					// $table1->activity_id = $activity_id;
			...
			// dd($activity);
			$msg4 = 'Operater '.$operator.' je uspesno promenio aktivnost';
			return view('OperatorLogin.change_secs' , compact('activity', 'msg4'));

		}

	// Logout
		public function logout_secs() {
			//
			...
			return view('OperatorLogin.logout_secs' /*, compact('activity')*/);
		}

		public function operator_logout_new_post_secs(Request $request) {
			...
			if (isset($alredy_logged_today[0]->id)) {
				...
				// dd($activity);
				$msg4 = 'Operater '.$table1->operator.' se uspesno odjavio';
				return view('OperatorLogin.logout_secs' , compact('activity', 'msg4'));
				...
				return view('OperatorLogin.logout_secs' , compact('activity', 'msg3'));
			}
